#ifndef LIB_EVENT_LOOP_HEADER
#define LIB_EVENT_LOOP_HEADER
 ...
 */
class LIB_PUBLIC_SYMBOL event_loop final : private thread
{
public:
...
private:
	...
	void LIB_PRIVATE_SYMBOL remove_handler(event_handler* handler);

	timer_id LIB_PRIVATE_SYMBOL add_timer(event_handler* handler, duration const& interval, bool one_shot);
	void LIB_PRIVATE_SYMBOL stop_timer(timer_id id);
	...
	bool LIB_PRIVATE_SYMBOL process_event(scoped_lock & l);

	// Process timers. Returns true if a timer has been triggered
	bool LIB_PRIVATE_SYMBOL process_timers(scoped_lock & l, monotonic_clock& now);

	virtual void LIB_PRIVATE_SYMBOL entry();
