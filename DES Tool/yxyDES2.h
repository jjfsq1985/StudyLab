...
private:
        byte szSubKeys[2][16][48];//储存2个16组48位密钥,第2个用于3DES
		byte szCiphertextRaw[64]; //储存二进制密文(64个Bits) int 0,1
		byte szPlaintextRaw[64]; //储存二进制密文(64个Bits) int 0,1
		byte szCiphertextInBytes[8];//储存8位密文
        byte szPlaintextInBytes[8];//储存8位明文字符串

		byte szCiphertextInBinary[65]; //储存二进制密文(64个Bits) char '0','1',最后一位存'\0'
		byte szCiphertextInHex[17]; //储存十六进制密文,最后一位存'\0'
		byte szPlaintext[9];//储存8位明文字符串,最后一位存'\0'

        byte szFCiphertextAnyLength[8192];//任意长度密文
        byte szFPlaintextAnyLength[8192];//任意长度明文字符串

		//功能:生成子密钥
        //参数:经过PC1变换的56位二进制字符串,生成的szSubKeys编号0-1
        //结果:将保存于byte szSubKeys[16][48]
