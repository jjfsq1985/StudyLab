...
void CDESToolDlg::OnBnClickedButtonDecrypt()
{
	...
	if(rdbRaw->GetCheck())
	{
		MessageBox(L"原始密文有不可显示字符,解密将不能得到正确结果.",L"警告",MB_OK);
		return;
	}
...
void CDESToolDlg::OnBnClickedButtonFileGoE()
{
	...
	{
		MessageBox(L"打不开源文件!",L"错误",MB_OK | MB_ICONERROR);
		return;
	}
	if ((fpDst = fopen(wcharToStr(szDstPath.GetBuffer()).c_str(), "wb")) == NULL)
	{
		MessageBox(L"打不开目的文件!",L"错误",MB_OK | MB_ICONERROR);
		return;
	}
	...
	fclose(fpDst);
	prcsbar.SetPos(100);
	MessageBox(L"加密完成!",L"提示",MB_OK | MB_ICONINFORMATION);
...
void CDESToolDlg::OnBnClickedButtonGoD()
{
	...
	fclose(fpDst);
	prcsbar.SetPos(100);
	MessageBox(L"解密完成!",L"提示",MB_OK | MB_ICONINFORMATION);
	prcsbar.ShowWindow(SW_HIDE);
}
