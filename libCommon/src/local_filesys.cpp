#include "../include/local_filesys.hpp"
...
namespace {
template<typename T>
int64_t make_int64libT(T hi, T lo)
{
...
local_filesys::type local_filesys::get_file_info(native_string const& path, bool &is_link, int64_t* size, datetime* modification_time, int *mode)
{
	...
	if (attributes.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
		...
		if (hFile != INVALID_HANDLE_VALUE) {
			...
			if (ret != 0 && !(info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT)) {
				...
				if (size)
                    *size = make_int64libT(info.nFileSizeHigh, info.nFileSizeLow);
	...
	if (is_dir) {
		...
		if (size)
            *size = make_int64libT(attributes.nFileSizeHigh, attributes.nFileSizeLow);
		return file;
	}
#else
	if (path.size() > 1 && path.back() == '/') {
		...
		if (is_link) {
			...
			if (size) {
				if (is_dir) {
                    ...
                    *size = make_int64libT(m_find_data.nFileSizeHigh, m_find_data.nFileSizeLow);
				}
			}
		}
		m_found = FindNextFile(m_hFind, &m_find_data) != 0;
