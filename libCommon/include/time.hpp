#ifndef LIB_TIME_HEADER
#define LIB_TIME_HEADER
...
class LIB_PUBLIC_SYMBOL duration;
...
*/
class LIB_PUBLIC_SYMBOL datetime final
{
public:
	/**
	...
	//datetime(datetime && op);
	datetime& operator=(datetime const& op) = default;
	//datetime& operator=(datetime && op);
	...
	friend duration LIB_PUBLIC_SYMBOL operator-(datetime const& a, datetime const& b);
...
private:
	int LIB_PRIVATE_SYMBOL compare_slow(datetime const& op) const;

	bool LIB_PRIVATE_SYMBOL clamped();

	enum invalid_t : int64_t {	invalid = 0 };
 ...
 */
class LIB_PUBLIC_SYMBOL duration final
{
public:
	duration() = default;
	...
	friend duration LIB_PUBLIC_SYMBOL operator-(duration const& a, duration const& b);
private:
 ...
 */
duration LIB_PUBLIC_SYMBOL operator-(datetime const& a, datetime const& b);
 ...
 */
class LIB_PUBLIC_SYMBOL monotonic_clock final
{
public:
	...
	//monotonic_clock& operator=(monotonic_clock &&);
