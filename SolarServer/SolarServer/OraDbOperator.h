...
public:
    bool OpenOraDb(const ostring& dbServerName, const ostring& cUser, const ostring& cPwd);
    bool CloseOraDb();

    int ExecuteCmd(const ostring& strSql);
    int ExecuteCmd(const ostring& strSql, Resultset& outResult);
    int ExecuteCmd(const ostring& strSql, DbParam* param, int nCount);
    int ExecuteCmd(const ostring& strSql, DbParam* param, int nCount, Resultset& outResult);
    int ExecuteProc(const ostring& cProcName);
    int ExecuteProc(const ostring& cProcName, Resultset& outResult);
    int ExecuteProc(const ostring& cProcName, DbParam* param, int nCount);
    int ExecuteProc(const ostring& cProcName, DbParam* param, int nCount, Resultset& outResult);
