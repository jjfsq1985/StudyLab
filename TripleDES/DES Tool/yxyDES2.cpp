...
const static byte PC1_Table[56] = {
...
const static byte PC2_Table[48] = {
...
const static byte Shift_Table[16] = {
	1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1
};
// initial permutation (IP)
const static byte IP_Table[64] = {
...
const static byte P_Table[32] = {
	16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23, 26, 5,  18, 31, 10,
	2,  8, 24, 14, 32, 27, 3,  9,  19, 13, 30, 6,  22, 11, 4,  25
};
// final permutation IP^-1 
const static byte IPR_Table[64] = {
...
void yxyDES2::InitializeKey(byte* srcBytes,unsigned int keyN)
{
...
void yxyDES2::CreateSubKey(byte* sz_56key,unsigned int keyN)
{
	byte szTmpL[28] = { 0 };
	byte szTmpR[28] = { 0 };
	byte szCi[28] = { 0 };
...
void yxyDES2::EncryptData(byte* _srcBytes,unsigned int keyN)
{
	byte szSrcBits[64] = {0};
	byte sz_IP[64] = {0};
	byte sz_Li[32] = {0};
	byte sz_Ri[32] = {0};
	byte sz_Final64[64] = {0};
...
void yxyDES2::DecryptData(byte* _srcBytes,unsigned int keyN)
{
	byte szSrcBits[64] = {0};
	byte sz_IP[64] = {0};
	byte sz_Li[32] = {0};
...
void yxyDES2::FunctionF(byte* sz_Li,byte* sz_Ri,unsigned int iKey,unsigned int keyN)
{
	byte sz_48R[48] = {0};
	byte sz_xor48[48] = {0};
	byte sz_P32[32] = {0};
...
void yxyDES2::InitialPermuteData(byte* _src,byte* _dst)
{
	//IP
	for(int i=0;i<64;i++)
	{
...
void yxyDES2::ExpansionR(byte* _src,byte* _dst)
{
	for(int i=0;i<48;i++)
	{
...
void yxyDES2::XOR(byte* szParam1,byte* szParam2, unsigned int uiParamLength, byte* szReturnValueBuffer)
{
...
void yxyDES2::CompressFuncS(byte* _src48, byte* _dst32)
{
	byte bTemp[8][6]={0};
	byte dstBits[4] = { 0 };
	for(int i=0;i<8;i++)
...
void yxyDES2::PermutationP(byte* _src,byte* _dst)
{
	for(int i=0;i<32;i++)
	{
...
void yxyDES2::Bytes2Bits(byte *srcBytes, byte* dstBits, unsigned int sizeBits)
{
	for(unsigned int i=0; i < sizeBits; i++)
		dstBits[i] = ((srcBytes[i>>3]<<(i&7)) & 128)>>7;
}

void yxyDES2::Bits2Bytes(byte *dstBytes, byte* srcBits, unsigned int sizeBits)
{
...
void yxyDES2::Int2Bits(unsigned int _src, byte* dstBits)
{
	for(unsigned int i=0; i < 4; i++)
		dstBits[i] = ((_src<<i) & 8)>>3;
}

void yxyDES2::Bits2Hex(byte *dstHex, byte* srcBits, unsigned int sizeBits)
{
...
void yxyDES2::Hex2Bits(byte *srcHex, byte* dstBits, unsigned int sizeBits)
{
...
byte* yxyDES2::GetCiphertextInBinary()
{
...
byte* yxyDES2::GetCiphertextInHex()
{
...
byte* yxyDES2::GetCiphertextInBytes()
{
	return szCiphertextInBytes;
}

byte* yxyDES2::GetPlaintext()
{
...
byte* yxyDES2::GetCiphertextAnyLength()
{
	return szFCiphertextAnyLength;
}

byte* yxyDES2::GetPlaintextAnyLength()
{
	return szFPlaintextAnyLength;
}

void yxyDES2::EncryptAnyLength(byte* _srcBytes,unsigned int _bytesLength,unsigned int keyN)
{
	if(_bytesLength == 8)
	{
...
void yxyDES2::DecryptAnyLength(byte* _srcBytes,unsigned int _bytesLength, unsigned int keyN)
{
	if(_bytesLength == 8)
	{
