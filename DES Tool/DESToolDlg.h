...
protected:
	...
	yxyDES2 *myDES;
	CBrush m_brush;
	byte bitsCiphertextAnyLength[32768];
	byte hexCiphertextAnyLength[16384];
	byte szSourceKey1[8];
	byte szSourceKey2[8];
	byte szPlaintextData[8192];
	byte szCiphertextData[8192];
	bool bIs3DES;
	void ConvertCiphertext2OtherFormat(int iBitsLen,byte *szCipherInBytes);
	int ConvertOtherFormat2Ciphertext(const char *szCipherInBytes);
