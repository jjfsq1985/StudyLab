...
bool do_is_routable_address(String const& address)
{
	...
	if (type == address_type::ipv6) {
		...
		if (long_address[0] == '0') {
			// ::/128
			if (long_address == libS(Char, "0000:0000:0000:0000:0000:0000:0000:0000")) {
				return false;
			}
			// ::1/128
            if (long_address == libS(Char, "0000:0000:0000:0000:0000:0000:0000:0001")) {
				return false;
			}

            if (long_address.substr(0, 30) == libS(Char, "0000:0000:0000:0000:0000:ffff:")) {
				Char const dot = '.';
				// IPv4 mapped
				String ipv4 =
        ...
        if (address.substr(0, 3) == libS(Char, "127") ||
            address.substr(0, 3) == libS(Char, "10.") ||
            address.substr(0, 7) == libS(Char, "192.168") ||
            address.substr(0, 7) == libS(Char, "169.254"))
		{
			return false;
		}

        if (address.substr(0, 3) == libS(Char, "172")) {
