#ifndef LIB_STRING_HEADER
#define LIB_STRING_HEADER
 ...
 *
 * Defines the \ref lib::native_string type and offers various functions to convert between
...
#endif
#if defined(LIB_UNIX) || defined(LIB_MAC)
 ...
 */
native_string LIB_PUBLIC_SYMBOL to_native(std::string const& in);

/** \brief Convert std::wstring to native_string.
 *
 * \return the converted string on success. On failure an empty string is returned.
 */
native_string LIB_PUBLIC_SYMBOL to_native(std::wstring const& in);
 ...
 */
int LIB_PUBLIC_SYMBOL stricmp(std::string const& a, std::string const& b);
int LIB_PUBLIC_SYMBOL stricmp(std::wstring const& a, std::wstring const& b);
...
template<>
std::wstring::value_type LIB_PUBLIC_SYMBOL tolower_ascii(std::wstring::value_type c);
 ...
 */
std::wstring LIB_PUBLIC_SYMBOL to_wstring(std::string const& in);

/// Returns identity, that way to_wstring can be called with native_string.
inline std::wstring LIB_PUBLIC_SYMBOL to_wstring(std::wstring const& in) { return in; }

/** \brief Converts from std::string in UTF-8 into std::wstring
 *
 * \return the converted string on success. On failure an empty string is returned.
 */
std::wstring LIB_PUBLIC_SYMBOL to_wstring_from_utf8(std::string const& in);
 ...
 * \note Does not handle embedded nulls
 */
std::string LIB_PUBLIC_SYMBOL to_string(std::wstring const& in);

/// Returns identity, that way to_string can be called with native_string.
inline std::string LIB_PUBLIC_SYMBOL to_string(std::string const& in) { return in; }
 ...
 * \note Does not handle embedded nulls
 */
std::string LIB_PUBLIC_SYMBOL to_utf8(std::string const& in);
 ...
 * \note Does not handle embedded nulls
 */
std::string LIB_PUBLIC_SYMBOL to_utf8(std::wstring const& in);
 ...
 *
 * Example: \c libT("this string is wide on Windows and narrow elsewhere")
 */
    #define libT(x) L ## x
#else
/** \brief Macro for a string literal in system-native character type.\n Note: Macro definition changes depending on platform!
 ...
 *       s += libS(String::value_type, "foo");
 *   }
 * \endcode
 */
#define libS(Char, s) choose_string<Char>(s, L ## s)

/// Returns \c in with all occurrences of \c find in the input string replaced with \c replacement
std::string LIB_PUBLIC_SYMBOL replace_substrings(std::string const& in, std::string const& find, std::string const& replacement);
std::wstring LIB_PUBLIC_SYMBOL replace_substrings(std::wstring const& in, std::wstring const& find, std::wstring const& replacement);

/// Modifies \c in, replacing all occurrences of \c find with \c replacement
void LIB_PUBLIC_SYMBOL replace_substrings(std::string& in, std::string const& find, std::string const& replacement);
void LIB_PUBLIC_SYMBOL replace_substrings(std::wstring& in, std::wstring const& find, std::wstring const& replacement);
